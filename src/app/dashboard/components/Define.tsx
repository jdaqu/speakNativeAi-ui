...
import { BookOpen, Volume2, Hash, Quote } from 'lucide-react'
...
export default function Define() {
  ...
  const handleWordClick = (word: string, definition?: string, example?: string) => {
    ...
    showLearningToast(word, 'vocabulary')
  }
  ...
  const handleSynonymClick = (synonym: string) => {
    learningPath.addItem({
      type: 'synonym',
      word: synonym,
      context: `Synonym of ${result?.word}`,
      source: 'define'
    })
    
    showLearningToast(synonym, 'synonym')
    setWord(synonym)
    setResult(null)
  }
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <h2 
                    ...
                    onClick={() => handleWordClick(result.word, 'Main word definition', result.definitions[0]?.example)}
              ...
              <CardContent>
                <div className="space-y-6">
                  {result.definitions.map((def, index) => (
                    <div 
                      key={index} 
                      className="border-l-4 border-primary pl-4 cursor-pointer hover:bg-gray-50 p-3 rounded-r-md transition-colors"
                      onClick={() => handleWordClick(result.word, def.definition, def.example)}
                      title="Click to add to learning path"
                    >
                ...
                <div className="flex flex-wrap gap-2">
                  {result.synonyms.map((synonym, index) => (
                    <span
                      ...
                      title="Click to add to learning path and look up"
                    >
                      {synonym}
                    </span>
                  ))}
